...
class Robot:
    ...
    def resetBody(self):
        ...
        if len(self.oldDebugInfo)>0:
            for x in self.oldDebugInfo:
               self.Lp.removeUserDebugItem(x)        
        self.Lp.resetBasePositionAndOrientation(self.quadruped, self.init_position,[0,0,0,1])
        self.Lp.resetBaseVelocity(self.quadruped, [0, 0, 0], [0, 0, 0])
        if(self.resetFunc):
            self.resetFunc()

    def checkSimulationReset(self,bodyOrn):
        (xr, yr, _) = self.Lp.getEulerFromQuaternion(bodyOrn)
    ...
    def getPos(self):
        bodyPos,_=self.Lp.getBasePositionAndOrientation(self.quadruped)
        return bodyPos
    ...
    def getIMU(self):
        _, bodyOrn = self.Lp.getBasePositionAndOrientation(self.quadruped)
        linearVel, angularVel = self.Lp.getBaseVelocity(self.quadruped)
